...
public class OpenapiV2Controller {
	...
	public ResponseEntity<String> doAddition(
		...
		Result result = null;
		String txId = txId();
		Span span = getSpan("api_extention", "add");
	...
	public ResponseEntity<String> doSubtraction(
		...
		String txId = txId();
		Result result = null;
		Span span = getSpan("api_extention", "sub");
		Scope scope = span.makeCurrent();
		setReqHeaders(span, request);
		try {
			validateRequest(txId, request);
	...
	public ResponseEntity<String> doDivision(
		...
		Result result = null;
		String txId = txId();
		Span span = getSpan("api_extention", "div");
		Scope scope = span.makeCurrent();
		setReqHeaders(span, request);
	...
	public ResponseEntity<String> doMultiply(
		...
		Result result = null;
		String txId = txId();
		Span span = getSpan("api_extention", "multi");
		Scope scope = span.makeCurrent();
		setReqHeaders(span, request);
		try {
			validateRequest(txId, request);
	...
	public ResponseEntity<String> doPower(
		...
		Result result = null;
		String txId = txId();
		Span span = getSpan("api_extention", "power");
		Scope scope = span.makeCurrent();
		setReqHeaders(span, request);
		try {
			validateRequest(txId, request);
	...
	private void validateRequest(String txid, HttpServletRequest request) throws OpenapiException {
		...
		Span span = getSpan("api_extention", "token_validation");
		Scope scope = span.makeCurrent();
		boolean ret = false;
		try {
