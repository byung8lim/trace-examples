...
public class AuthV1Controller {
	...
	public ResponseEntity<String> getAllList(
		...
		String txId = txId();
		Result result = null;
		Span span = this.startServerSpan("auth.basic.list", request);
	...
	public ResponseEntity<String> createAppKey(
		...
		String txId = txId();
		Result result = null;
		Span span = this.startServerSpan("auth.basic.issue", request);
		try (Scope scope = tracer.scopeManager().activate(span)) {
		...
		Map<String,String> fields = new LinkedHashMap<>();
		fields.put("type", reqData.getType());
		fields.put("appName", reqData.getAppName());
		fields.put("orgNo", reqData.getOrgNo());
		span.log(fields);
		span.setTag("response", result.toJson());
		return new ResponseEntity<String>(result.toJson(), result.status());
	}
	...
	public ResponseEntity<String> vaildateAppKey(
		...
		String txId = txId();
		Result result = null;
		Span span = this.startServerSpan("auth.basic.validate", request);
		try (Scope scope = tracer.scopeManager().activate(span)) {
			try {
	...
	synchronized private boolean hasKey(String key, Span parent) {
		boolean ret = false;
		String msg = null;
		Span span = tracer.buildSpan("auth.basic.hasKey").asChildOf(parent).start();
		try {
			Map<String, String> fields = new LinkedHashMap<>();
			fields.put("key", key);
