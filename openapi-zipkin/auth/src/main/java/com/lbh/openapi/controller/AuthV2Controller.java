...
public class AuthV2Controller {
	...
	public ResponseEntity<String> getAllList(
		...
		if (traceId == null || spanId == null) {
		    ...
		    newTrace.name("auth.extention.list");
		    newTrace.start();
	    ...
	    newSpan.name("auth.extention.list");
	    newSpan.start();
	...
	public ResponseEntity<String> createAppKey(
		...
		if (traceId == null || spanId == null) {
			newTrace = tracer.newTrace();
		    parent = newTrace.context();
		    newTrace.name("auth.extention.issue");
		    newTrace.start();
	    ...
	    newSpan = this.tracer.newChild(parent);
	    newSpan.name("Vauth.extention.issue");
	    newSpan.start();
		newSpan.tag("Headers", getHeaders(request));
		try {
	...
	public ResponseEntity<String> vaildateAppKey(
		...
		if (traceId == null || spanId == null) {
			newTrace = tracer.newTrace();
		    parent = newTrace.context();
		    newTrace.name("auth.extention.validate");
		    newTrace.start();
	    ...
	    newSpan = this.tracer.newChild(parent);
	    newSpan.name("auth.extention.validate");
	    newSpan.start();
		newSpan.tag("Headers", getHeaders(request));
		try {
